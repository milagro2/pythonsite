...
{
  "id" : "ba_GenCopyAttributeValuesForPackUnit",
  "type" : "BusinessAction",
...
*/
exports.operation0 = function (node,atg_TIIPack_AlwaysOverwrite,bf_GetDimensionAttributesFromGroup,bf_GetGlobalAttributesFromGroup,ptp_PartnerArticle,manager) {
...
node.queryReferences(ptp_PartnerArticle).forEach(function (tradeItemRef) {
	...
	for (var i=0; i < attributeGroups.size(); i++) {
		...
		var globalAttributes = bf_GetGlobalAttributesFromGroup.evaluate({attributeGroup: atg_TIIPack_AlwaysOverwrite});
		for (var j=0; j < globalAttributes.size(); j++) {
			if (globalAttributes.get(j).isMultiValued()) {
				...
				//If trade item has data in multivalue, clear multivalue from PU before adding.
				if(globalMultiValues.length() > 0) {
		...
		//Copy all language dependent attribute values in all relevant contexts for the attributes which are in the group(s)
		var dimensionAttributes = bf_GetDimensionAttributesFromGroup.evaluate({attributeGroup: atg_TIIPack_AlwaysOverwrite});
