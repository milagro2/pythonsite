...
{
  "id" : "ba_RefreshPartnerImageAndDownload",
  "type" : "BusinessAction",
  "setupGroups" : [ "brg_AssetRelatedActions" ],
  "name" : "Refresh Partner Image And Download",
  ...
  "scope" : "Global",
  "validObjectTypes" : [ "prd_Article", "prd_PackagingUnitCase", "prd_PackagingUnitPack", "prd_PackagingUnitPallet" ],
...
*/
exports.operation0 = function (node,assetDownloadHome,AssetDownload_AssetURLAttribute,att_AssetDownloadError,logger,ref_PartnerProductImage,prd_Level1Partner,manager,PartnerItems,ptp_PartnerArticle) {
/* 
...
*/
//logger.info("check deze case"+node)
node.queryReferences(ptp_PartnerArticle).forEach(function(reference){
	var tradeItem = reference.getTarget();
	logger.info(tradeItem);
	
	if (tradeItem.getParent().getObjectType().getID()==prd_Level1Partner.getID()){
		var tiparentlvl1 = manager.getClassificationHome().getClassificationByID(tradeItem.getParent().getID());
	}
	if (tradeItem.getParent().getParent().getObjectType().getID()==prd_Level1Partner.getID()){
		var tiparentlvl1 = manager.getClassificationHome().getClassificationByID(tradeItem.getParent().getParent().getID());
	} 
	if (tradeItem.getParent().getParent().getParent().getObjectType().getID()==prd_Level1Partner.getID()){
		var tiparentlvl1 = manager.getClassificationHome().getClassificationByID(tradeItem.getParent().getParent().getParent().getID());
	}
	
	var itr = tradeItem.getDataContainerByTypeID("dct_ProductAssetInformation").getDataContainers().iterator();
	while (itr.hasNext()) {
			...
			if (referencedAsset.hasContent()){
				...
				if (url) {
					...
					try {
						...
						referencedAsset.getValue(att_AssetDownloadError.getID()).setSimpleValue(error.message);
