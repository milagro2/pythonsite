...
{
  "id" : "ba_CreatePackagingUnitPack",
  "type" : "BusinessAction",
  "setupGroups" : [ "brg_Actions" ],
  "name" : "Create Packaging Unit [Pack] when a new Article is initiated",
  "description" : null,
  "scope" : "Global",
...
/*===== business rule plugin definition =====
{
  ...
  "messages" : [ {
    ...
    "message" : "The enter EAN Nummer is already in use in the following {article} {name}. Please select a different EAN Nummer. ",
    "translations" : [ ]
  } ],
  "pluginType" : "Operation"
}
*/
exports.operation0 = function (node,manager,prd_PackagingUnitPack,att_PackName,att_GlobalTradeIdentificationNumber,att_PackGTIN,ba_CreateNewGTIN,prd_PackagingUnitEach,ref_NextLowerLevel,att_QuantityOfNextLowerLevel,ref_GlobalTradeIdentificationNumbers,ErrorDuplicate) {
...
var packGTIN = node.getValue(att_PackGTIN.getID()).getSimpleValue();
var found = false;
if(packGTIN){
	...
	packagingUnitPack.getValue(att_GlobalTradeIdentificationNumber.getID()).setSimpleValue(packGTIN);
		
	var gtin = '00000000000000'.substring(1,15-packGTIN.length()) + packGTIN;
	var gtinObject = manager.getNodeHome().getObjectByKey('key_GTINObject', gtin);
	logger.info(gtin);
	logger.info(gtinObject);
	if (!gtinObject) {
		ba_CreateNewGTIN.execute(packagingUnitPack);
	}else{
