...
{
  "id" : "ba_CopyDataContainersForPackUnit",
  "type" : "BusinessAction",
...
/*===== business rule plugin definition =====
{
  ...
  "binds" : [ {
    ...
    "alias" : "visualCheckAttributeGroupAttribute",
...
*/
exports.operation0 = function (node,alwaysCopyAttributeGroup,visualCheckAttributeGroupAttribute,getDimensionAttributesFromGroup,getGlobalAttributesFromGroup,tradeItemLinkReferenceType,getKeyAttributesForDataContainerType,getDataContainerFromGroup,manager) {
...
var visualCheckAttributeValues = node.getValue(visualCheckAttributeGroupAttribute.getID()).getValues().toArray();
...
node.queryReferences(tradeItemLinkReferenceType).forEach(function (tradeItemRef){
	...
	for(var i=0; i < attributeGroups.size();i++){
		...
		var dataContainerTypes = getDataContainerFromGroup.evaluate({attributeGroup:attributeGroups.get(i)});						
		for(var j=0; j < dataContainerTypes.size();j++){	
			//Copy Data container	
			log.info(node.getID()+' '+dataContainerTypes.get(j).getID());
...
function createDataContainerWithKey(dataContainerType,sourceDataContainerObject,target,isSingleDataContainer,target){
	...
	try{
		if(isSingleDataContainer){
			...
			log.info(target.getID()+"<--target id");
			log.info(target.getDataContainerByTypeID(dataContainerType.getID())+"<--target data container");
