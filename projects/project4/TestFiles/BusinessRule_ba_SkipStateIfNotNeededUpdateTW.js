...
{
  "id" : "ba_SkipStateIfNotNeededUpdateTW",
  "type" : "BusinessAction",
  "setupGroups" : [ "brg_WorkflowProcessing" ],
  "name" : "Skip State If Not Needed (Update Text Writer)",
  "description" : null,
  "scope" : "Global",
...
for(var z in work){
	...
	if(work[z].getLOVValue().getID().equals("Copywriter")){
		found = true;
	}	
}
...
if(found == false){
	...
	var triggerResult = node.getTaskByID("wf_UpdateExistingArticle","ExternalTextWriterUpdate").triggerLaterByID("Done","Triggered by Business rule");
	}
}
}